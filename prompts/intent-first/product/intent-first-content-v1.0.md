...
**Codebase-First Focus:** Update existing content before creating new. Build upon messaging that's already working.
...
- **Existing Content Fit**: Can we enhance what's already there?
...
- **Existing content could be updated instead**
...
| Audience Impact | Business Impact | Content Effort | Existing Content | Priority |
|----------------|----------------|---------------|------------------|----------|
| High | High | Low-Medium | Needs Update | **Critical – Update immediately** |
| High | Medium | Low-Medium | Needs Update | **High – Update this sprint** |
| Medium | High | Low | Needs Update | **Medium – Plan for next iteration** |
| High | High | High | None | **Create new content** |
| High | High | Any | Strong | **Verify existing, minor updates if needed** |
| Low | Low | Any | Any | **Skip or create minimal reference** |
...
4. **Maintain consistency** - ensure tone and messaging align
5. **Measure performance** - track what content actually achieves
...
2. **Existing Content Review**:
   ...
   - How is current content performing?
   - Can existing content be updated instead?

3. **Search and Consumption Patterns**:
...
- Update existing content before creating new
```
...
- **"Can we update existing content instead?"**

---

## Related

- See `intent-first-product-v1.0.md` for product messaging
- See `intent-first-documentation-v1.0.md` for technical content
