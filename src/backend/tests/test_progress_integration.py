...
async def test_progress_batch_integration(client, auth_headers, db_session):
    ...
    resp1 = await client.post("/api/v1/progress/batch", json={"profile_id": profile_id, "items": items}, headers=auth_headers)
    ...
    resp2 = await client.post("/api/v1/progress/batch", json={"profile_id": profile_id, "items": items}, headers=auth_headers)
    ...
    resp3 = await client.post("/api/v1/progress/batch", json={"profile_id": profile_id, "items": [items[0], new_item]}, headers=auth_headers)
