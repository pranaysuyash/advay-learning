...
test.describe('AlphabetGame Hand Tracing', () => {
  ...
  test('Home page loads correctly', async ({ page }) => {
    await page.goto('http://localhost:6173/', { waitUntil: 'domcontentloaded' });
  ...
  test('Games list page shows available games', async ({ page }) => {
    await page.goto('http://localhost:6173/games', { waitUntil: 'domcontentloaded' });
    await page.waitForTimeout(2000);
  ...
  test('Dashboard page loads', async ({ page }) => {
    await page.goto('http://localhost:6173/dashboard', { waitUntil: 'domcontentloaded' });
  ...
  test('Settings page loads', async ({ page }) => {
    await page.goto('http://localhost:6173/settings', { waitUntil: 'domcontentloaded' });
    await page.waitForTimeout(1000);
  ...
  test('No technical delegate info leaked to UI on any page', async ({ page }) => {
    const pages = ['/dashboard', '/games', '/settings'];
    
    for (const path of pages) {
      await page.goto(`http://localhost:6173${path}`, { waitUntil: 'domcontentloaded' });
  ...
  test('Navigation between pages works', async ({ page }) => {
    ...
    if (overlayCount > 0 && await overlay.first().isVisible({ timeout: 1000 })) {
  ...
  test('App shows feedback/status messages', async ({ page }) => {
    ...
    const statusElements = page.locator('[role="status"]').or(page.locator('[aria-live]'));
