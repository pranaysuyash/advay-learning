...
async function debugMirrorMaze() {
  ...
  try {
    ...
    const domStructure = await page.evaluate(() => {
      ...
      if (gameContainer) {
        return {
          ...
          innerHTML: gameContainer.innerHTML.substring(0, 500),
        };
      }
      return { found: false };
    });
    ...
    // Check if any React error boundaries are present
    const hasErrorBoundary = await page.isVisible('[class*="error"], [class*="Error"]').catch(() => false);
