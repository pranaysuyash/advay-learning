 ...
 *
 * Also provides keyboard fallback when hand tracking is unavailable.
 */
export class HandInteraction {
  ...
  // keyboard fallback is not currently used but retained for future expansion
  private keyboardPosition: Vector2 = { x: 0, y: 0 };
  private keyboardStep: number = 24;
  ...
  /**
   * Set keyboard position for fallback input
   */
  setKeyboardPosition(x: number, y: number): void {
    this.keyboardPosition = { x, y };
    // keyboardActive flag was previously used for fallback input but is unused now
  }

  /**
   * Move keyboard cursor in direction
   */
  moveKeyboardCursor(direction: 'left' | 'right' | 'up' | 'down'): void {
    switch (direction) {
      case 'left':
        this.keyboardPosition.x = Math.max(
          0,
          this.keyboardPosition.x - this.keyboardStep,
        );
        break;
      case 'right':
        this.keyboardPosition.x = this.keyboardPosition.x + this.keyboardStep;
        break;
      case 'up':
        this.keyboardPosition.y = Math.max(
          0,
          this.keyboardPosition.y - this.keyboardStep,
        );
        break;
      case 'down':
        this.keyboardPosition.y = this.keyboardPosition.y + this.keyboardStep;
        break;
    }
  }
