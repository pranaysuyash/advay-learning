...
const CATEGORY_COLORS: Record<string, { bg: string; text: string; border: string }> = {
    ...
    'default': { bg: 'bg-slate-100', text: 'text-advay-slate', border: 'border-[#F2CC8F]' },
};
        ...
        >
            ...
            >
                ...
                <div className="relative h-40 bg-slate-50 overflow-hidden border-b-4 border-[#F2CC8F] shrink-0">
                    ...
                    {/* Stars display */}
                    {starsEarned > 0 && (
                        <div className="absolute top-4 right-4 flex gap-1 bg-white/90 backdrop-blur-sm px-2 py-1 rounded-full shadow-[0_4px_0_#E5B86E] border-2 border-yellow-100">
                            {[1, 2, 3].map((star) => (
                                <span
                                    key={star}
                                    className={`text-xl -mt-1 drop-shadow-[0_4px_0_#E5B86E] ${star <= starsEarned ? 'text-yellow-400' : 'text-slate-200'}`}
                                >
                                    ★
                                </span>
                            ))}
                ...
                <div className="p-5 flex-1 flex flex-col">
                    <h3 className="text-2xl font-black mb-2 text-advay-slate group-hover:text-[#3B82F6] transition-colors leading-tight tracking-tight">
                        {title}
                    </h3>

                    <p className="text-advay-slate font-semibold text-sm mb-5 line-clamp-2 leading-relaxed flex-1">
                        {description}
                    </p>
