...
const WellnessDashboard: React.FC<WellnessDashboardProps> = ({
  ...
  const { lastAttention, isLoading: _attentionLoading } =
  ...
  return (
    <div className='bg-white/10 border border-border rounded-xl p-6 shadow-sm'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 gap-6 mb-6'>
        <div
          className={`border ${postureStatus.borderColor} rounded-xl p-5 ${postureStatus.bgColor}`}
        >
        ...
        <div
          className={`border ${attentionStatus.borderColor} rounded-xl p-5 ${attentionStatus.bgColor}`}
        >
          <div className='flex items-center gap-3 mb-3'>
        ...
        <ul className='space-y-2 text-sm text-white/80'>
          {metrics.postureScore < 70 && (
            <li className='flex items-start gap-2'>
              ...
              <span>
                Encourage your child to sit up straight with back against chair
              </span>
            </li>
          )}
          {metrics.attentionScore < 70 && (
            <li className='flex items-start gap-2'>
              ...
              <span>
                Your child might benefit from a short break to refresh focus
              </span>
            </li>
          )}
          ...
          <div className='space-y-4'>
            <div className='bg-white/5 border border-border rounded-lg p-4'>
              <div className='flex justify-between items-center mb-2'>
                ...
                <span
                  className={`px-2 py-1 rounded-full text-xs ${postureStatus.color} ${postureStatus.bgColor}`}
                >
                  {postureStatus.status.replace('_', ' ')}
                </span>
              </div>
              <p className='text-sm text-white/60'>
                Shoulder alignment:{' '}
                {lastPosture
                  ? Math.round(lastPosture.shoulderAlignment * 100)
                  : '--'}
                % | Spine curvature:{' '}
                {lastPosture
                  ? Math.round(lastPosture.spineCurvature * 100)
                  : '--'}
                %
              </p>
            </div>
            ...
            <div className='bg-white/5 border border-border rounded-lg p-4'>
              <div className='flex justify-between items-center mb-2'>
                ...
                <span
                  className={`px-2 py-1 rounded-full text-xs ${attentionStatus.color} ${attentionStatus.bgColor}`}
                >
                  {attentionStatus.status.replace('_', ' ')}
                </span>
              </div>
              <p className='text-sm text-white/60'>
                Focus level:{' '}
                {lastAttention
                  ? Math.round(lastAttention.focusLevel * 100)
                  : '--'}
                % | Blink rate: {lastAttention ? lastAttention.blinkRate : '--'}{' '}
                per min
              </p>
            </div>
          </div>
