...
const ColorByNumberGame = memo(function ColorByNumberGameComponent({
  completeGame,
}: ColorByNumberGameProps) {
  ...
  const [gameState, setGameState] = useState(() =>
    createInitialState(activeTemplate),
  );
  const [unlockedLevel, setUnlockedLevel] = useState(0);
  const [completedLevelStars, setCompletedLevelStars] = useState<
    Record<number, number>
  >({});
  const [feedback, setFeedback] = useState(
    'Pick a color number and tap matching regions.',
  );
  ...
  const handleSelectColor = (number: number) => {
    ...
    setFeedback(
      `Color ${number} selected${palette ? `: ${palette.label}` : ''}.`,
    );
    playClick();
  };
    ...
    setGameState((prev) => {
      ...
      if (outcome.result === 'correct') {
        if (outcome.state.completed) {
          ...
          setCompletedLevelStars((prev) => ({
            ...prev,
            [templateIndex]: summary.stars,
          }));
          setUnlockedLevel((prev) =>
            Math.min(
              COLOR_BY_NUMBER_TEMPLATES.length - 1,
              Math.max(prev, templateIndex + 1),
            ),
          );
          setFeedback(
  ...
  if (view === 'menu') {
    return (
      <GameContainer
        ...
        score={Object.values(completedLevelStars).reduce(
          (acc, stars) => acc + stars,
          0,
        )}
        ...
        <div className='h-full overflow-auto p-4 md:p-6'>
          <div className='relative max-w-5xl mx-auto space-y-5'>
            ...
            <section className='rounded-3xl border-2 border-[#F2CC8F] p-6 bg-gradient-to-br from-[#FEF3C7] via-white to-[#DBEAFE] shadow-[0_6px_0_#E5B86E]'>
              <p className='text-sm font-black uppercase tracking-widest text-[#B45309]'>
                New Game
              </p>
              <h2 className='text-4xl font-black text-slate-900 mt-2'>
                Color by Number Adventure
              </h2>
              <p className='text-lg font-bold text-slate-700 mt-3'>
                Choose a picture, match numbers to colors, and unlock the next
                level with stars.
              </p>
              <div className='mt-4 flex flex-wrap gap-2 text-sm font-bold text-slate-700'>
                <span className='px-3 py-1 bg-white rounded-full border border-[#F2CC8F]'>
                  Match numbers
                </span>
                <span className='px-3 py-1 bg-white rounded-full border border-[#F2CC8F]'>
                  Build streak bonus
        ...
        <div className='max-w-6xl mx-auto space-y-4 md:space-y-6'>
          <section className='bg-white rounded-2xl border-2 border-[#F2CC8F] p-4 shadow-[0_4px_0_#E5B86E]'>
            <div className='flex flex-wrap gap-2 items-center justify-between'>
              <div>
                ...
                <p className='text-sm font-bold text-slate-600 mt-1'>
                  Level {templateIndex + 1} of{' '}
                  {COLOR_BY_NUMBER_TEMPLATES.length}
                </p>
              </div>
            ...
            <div className='mt-4'>
              ...
              <p className='mt-2 text-sm font-bold text-slate-600'>
                Progress: {completion}% • Mistakes: {gameState.mistakes} •
                Moves: {gameState.moves} • Best streak: {gameState.maxStreak}
              </p>
                ...
                return (
                  ...
                  >
                    ...
                    <p className='text-xs font-bold text-slate-600 mt-1'>
                      Remaining:{' '}
                      {getRemainingCountByNumber(gameState, entry.number)}
                    </p>
                  </button>
                );
              })}
            ...
            <div className='grid grid-cols-2 md:grid-cols-4 gap-3'>
              {gameState.regions.map((region) => {
                const palette = COLOR_PALETTE.find(
                  (entry) => entry.number === region.number,
                );
                return (
                  <button
                    key={region.id}
          ...
          {gameState.completed && (
            <section className='bg-[#10B981]/10 rounded-2xl border-2 border-[#10B981] p-5'>
              <h3 className='text-2xl font-black text-[#047857]'>
                Picture complete!
              </h3>
              <p className='font-bold text-slate-700 mt-2'>
                Final score: {gameState.score} • Mistakes: {gameState.mistakes}{' '}
                • Moves: {gameState.moves}
              </p>
              ...
              <div className='mt-4 flex flex-wrap gap-3'>
                ...
                <button
                  type='button'
                  disabled={
                    templateIndex >= COLOR_BY_NUMBER_TEMPLATES.length - 1
                  }
                  onClick={() =>
                    startLevel(Math.min(templateIndex + 1, unlockedLevel))
                  }
