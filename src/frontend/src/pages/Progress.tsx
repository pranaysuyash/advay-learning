  ...
  const letterProgress = progress
    ...
    .reduce((acc, item) => {
      ...
      if (existing) {
        ...
        existing.status = existing.accuracy >= 80 ? 'completed' : 'in_progress';
      } else {
        acc.push({
          letter: item.content_id,
          accuracy: item.score,
          status: item.score >= 80 ? 'completed' : item.score > 0 ? 'in_progress' : 'locked',
        });
      }
      return acc;
    }, [] as { letter: string; accuracy: number; status: 'completed' | 'in_progress' | 'locked' }[]);
        ...
        {!loading && !error && stats && (
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
              ...
              <div className="bg-white/5 border border-white/10 rounded-xl p-6 text-center">
                <div className="text-3xl font-bold">{stats.completion_count}</div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                  {letterProgress.map((item) => (
                    ...
                    >
                      ...
                      {item.accuracy > 0 && (
                        <div className="text-sm mt-1">{Math.round(item.accuracy)}%</div>
                      )}
                    </div>
                  ))}
                </div>
