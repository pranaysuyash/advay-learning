...
export function AirGuitarHero() {
  ...
  const handleFinish = useCallback(async () => {
    playClick();
    const finalScore = Math.round(score / Math.max(levelConfig.notesToPlay, 1));
    await onGameComplete(finalScore);
    navigate('/games');
  }, [score, levelConfig, onGameComplete, navigate, playClick]);

  const currentNote = noteSequence[currentIndex];
  const noteColors = currentNote ? (NOTE_COLORS[currentNote.name] ?? NOTE_COLORS.G) : NOTE_COLORS.G;
        ...
        <div className='max-w-2xl mx-auto space-y-4'>
          ...
          <div className='flex gap-2 justify-center'>
            {LEVELS.map((level) => (
              <button
                key={level.level}
                type='button'
                onClick={() => { playClick(); setCurrentLevel(level.level); setGameState('start'); }}
                className={`px-5 py-2 rounded-full font-black text-sm transition-all shadow-[0_3px_0_#6D28D9] ${currentLevel === level.level
          ...
          {gameState === 'start' && (
            <div className='flex flex-col items-center gap-6 bg-white rounded-3xl border-3 border-[#F2CC8F] p-10 shadow-[0_6px_0_#E5B86E] text-center'>
              ...
              <div>
                <h2 className='text-4xl font-black text-slate-900 tracking-tight'>Air Guitar Hero!</h2>
              ...
              <div className='grid grid-cols-3 gap-2'>
                {['E', 'A', 'D', 'G', 'B', 'e'].map((note) => (
                  <span
                    ...
                    style={{ backgroundColor: NOTE_COLORS[note]?.bg, borderColor: NOTE_COLORS[note]?.border, color: NOTE_COLORS[note]?.text }}
                  >
                    {note} string
                  </span>
                ))}
          ...
          {gameState === 'playing' && currentNote && (
            <>
              ...
              <div
                ...
                style={{ backgroundColor: noteColors.bg, borderColor: noteColors.border }}
              >
                <p className='text-sm font-black uppercase tracking-widest mb-2' style={{ color: noteColors.text }}>
                  Note {currentIndex + 1} of {noteSequence.length}
                </p>
                <p className='text-6xl font-black mb-3' style={{ color: noteColors.text }}>
                  {currentNote.name}
                </p>
              ...
              <div className='bg-gradient-to-b from-slate-800 to-slate-900 rounded-2xl p-4 relative overflow-hidden border-3 border-slate-700 shadow-2xl'>
                ...
                {['E', 'A', 'D', 'G', 'B', 'e'].map((string, _idx) => {
                  ...
                  return (
                    ...
                    >
                      <span className='text-xs font-black w-4' style={{ color: colors.border }}>{string}</span>
                      <div
                        className={`flex-1 rounded-full transition-all ${isCurrent ? 'h-1.5' : 'h-0.5'}`}
                        style={{ backgroundColor: isCurrent ? colors.border : '#475569' }}
                      />
                    </div>
                  );
                })}
                ...
                <div className='absolute inset-y-0 flex gap-16 pointer-events-none ml-8'>
                  {[1, 2, 3, 4].map((f) => (
                    <div key={f} className='w-0.5 h-full bg-slate-600 opacity-50' />
                  ))}
                </div>
              </div>
              ...
              <button
                ...
                className={[
                  ...
                  strumAnimating ? 'scale-95' : 'hover:scale-105 active:scale-95',
              ...
              <div className='flex gap-1.5 flex-wrap justify-center'>
                {noteSequence.map((note, idx) => {
                  ...
                  return (
                    <div
                      key={`${note.id}-${idx}`}
                      className={`w-9 h-9 rounded-full flex items-center justify-center font-black text-xs border-2 transition-all ${idx < currentIndex
                          ? 'bg-emerald-100 border-emerald-400 text-emerald-700'
                          : idx === currentIndex
                            ? 'scale-125 border-2 animate-pulse'
                            : 'opacity-30 border-slate-200 bg-slate-50 text-slate-500'
                        }`}
                      style={idx === currentIndex ? { backgroundColor: colors.bg, borderColor: colors.border, color: colors.text } : {}}
                    >
                      {note.name}
                    </div>
                  );
              ...
              <div className='flex items-center justify-between'>
                <div className='flex gap-3'>
                  <div className='bg-emerald-50 border-2 border-emerald-200 px-4 py-2 rounded-xl text-center'>
                    <p className='text-xs font-black uppercase text-emerald-600'>Played</p>
          ...
          {gameState === 'complete' && (
            <div className='flex flex-col items-center gap-5 bg-white rounded-3xl border-3 border-[#F2CC8F] p-10 shadow-[0_6px_0_#E5B86E] text-center'>
              ...
              <p className='text-lg text-slate-600 font-bold'>You shredded {correctCount} notes!</p>
