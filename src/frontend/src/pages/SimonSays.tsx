...
export function SimonSaysHeader({
  score,
  onBack,
}: {
  score: number;
...
export function SimonSaysCelebration({
  showCelebration,
}: {
      ...
      <div className='bg-[#FFF8F0] border-3 border-[#F2CC8F] rounded-[2rem] p-8 mb-12 max-w-2xl w-full text-left'>
        ...
        <ul className='space-y-4 text-advay-slate font-bold text-lg'>
          <li className='flex items-center gap-3'>
            <Camera className='w-8 h-8 text-blue-500' /> Stand directly in front
            of your camera
          </li>
          <li className='flex items-center gap-3'>
            <Ear className='w-8 h-8 text-pink-500' /> Listen carefully to what
            Simon says
          </li>
          <li className='flex items-center gap-3'>
            <span className='inline-block w-8' /> Do the action with your whole
            body!
          </li>
          <li className='flex items-center gap-3'>
            <span className='inline-block w-8' /> Hold the pose steady to
            complete it
          </li>
          {gameMode === 'combo' && (
            <li className='flex items-center gap-3'>
              <Hand className='w-8 h-8 text-purple-500' /> Also show the correct
              number of fingers!
            </li>
          )}
        </ul>
      ...
      >
        <div className='text-center mb-10'>
          ...
          <div className='flex justify-center mb-4'>
            <KenneyCharacter
              type='beige'
              animation={getCharacterAnimation(currentAction.name)}
              size='lg'
            />
          </div>
        ...
        <div className='space-y-6 mt-auto'>
          <div>
            <div className='flex justify-between font-bold text-text-secondary mb-2 uppercase tracking-wide text-sm'>
              ...
              <span className={matchProgress > 70 ? 'text-[#10B981]' : ''}>
          ...
          <div>
            <div className='flex justify-between font-bold text-text-secondary mb-2 uppercase tracking-wide text-sm'>
              ...
              <span className={holdPercentage >= 100 ? 'text-amber-500' : ''}>
                {Math.round(holdPercentage)}%
              </span>
            </div>
            <div className='h-6 bg-slate-100 rounded-full overflow-hidden border-2 border-[#F2CC8F]/50 p-1'>
              <motion.div
...
const SimonSaysContent = memo(function SimonSays() {
  ...
  const detectPose = useCallback(() => {
    ...
    if (results.landmarks && results.landmarks.length > 0) {
      ...
      const matchScore = calculatePoseMatchScore(
        landmarks,
      ...
      const fingerMatches = checkFingerMatch(
        gameMode,
        targetFingers,
        detectedFingers,
      );
