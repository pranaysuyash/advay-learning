...
function PackLunchboxGame() {
  ...
  const handleFrame = useCallback(
    (frame: TrackedHandFrame, _meta: HandTrackingRuntimeMeta) => {
      ...
      setCursor(tip);
      // Update pinch state from frame
      setIsPinching(frame.pinch?.state?.isPinching ?? false);
  ...
  const initializeLevel = useCallback(() => {
    ...
    const newItems: DraggableItem[] = availableFoods.map((food, idx) => ({
      ...
      y:
        screenHeight * 0.15 +
        Math.floor(idx / itemsPerRow) * (itemSize + itemMargin),
      size: itemSize,
      content: food.emoji,
      color: food.color,
      data: food,
  ...
  useEffect(() => {
    if (gameState === 'playing') {
      initializeLevel();
      speakText(
        'Put the food in the lunchbox! One fruit, one vegetable, and one protein!',
      );
    }
    ...
    (item: DraggableItem, _zone: DropZone) => {
      ...
      if (newLunchboxItems.length >= LUNCHBOX_SLOTS) {
        ...
        if (
          result.treats > 0 &&
          result.treats >=
            newLunchboxItems.filter((f) => f.category === 'treat').length
        ) {
        ...
        <div className='flex flex-col items-center justify-center h-full p-4'>
          ...
          >
            ...
            <p className='text-xl text-green-600 mb-8 max-w-md'>
              Put healthy foods in your lunchbox! Try to include fruit,
        ...
        <div className='relative h-full'>
          ...
          <div className='flex justify-between items-center p-4'>
            ...
            <div className='flex gap-2'>
              ...
              {[...Array(LUNCHBOX_SLOTS - lunchboxItems.length)].map(
                (_, idx) => (
                  <span
                    key={`empty-slot-${lunchboxItems.length + idx}`}
