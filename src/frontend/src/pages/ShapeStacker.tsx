...
function ShapeStackerContent() {
  ...
  const FALL_SPEED =
    currentLevel === 1 ? 0.05 : currentLevel === 2 ? 0.08 : 0.12;
  ...
  const startGame = useCallback(() => {
    ...
    if (ttsEnabled) {
      speak(
        'Tap the falling shapes to drop them into the matching slots below!',
      );
    }
  ...
  const handleShapeClick = useCallback(
    (shape: FallingShape) => {
      ...
      const availableTarget = targets.find(
        (t) => !t.filled && checkMatch(shape, t),
      );
      ...
      if (availableTarget) {
        ...
        setFeedback(
          `Great match! ${newStreak > 1 ? `✨ ${newStreak}x Streak!` : ''}`,
        );
        playSuccess();
    ...
    return (
      ...
      >
        <div className='flex flex-col items-center justify-center h-full gap-6 p-8 relative'>
          <VoiceInstructions
            text='Tap the falling shapes to match them with the target slots!'
            autoPlay={true}
          />
          ...
          <p className='text-xl text-slate-700 font-bold text-center max-w-lg mb-4 bg-white/50 p-6 rounded-2xl border-2 border-indigo-100 shadow-sm'>
            Watch carefully! As shapes fall from the top,{' '}
            <span className='text-indigo-600'>tap</span> them to drop them into
            the matching empty slots at the bottom. Fill all slots to win!
          </p>
      ...
      >
        <div className='flex flex-col items-center justify-center h-full gap-6 p-8'>
          ...
          <p className='text-lg text-slate-600'>
            Matches: {matches}/{targets.length}
          </p>
        ...
        <GameHUD
          ...
          rightHeaderContent={
            <div className='flex gap-4 items-center'>
              ...
              <div
                className={`bg-white/90 backdrop-blur-sm px-4 py-1.5 rounded-xl font-black border-2 shadow-sm text-sm ${timeLeft <= 10 ? 'text-red-500 animate-pulse border-red-200' : 'text-slate-600 border-slate-200'}`}
              >
                ⏱️ {timeLeft}s
              </div>
            </div>
        ...
        <div className='absolute bottom-[30%] left-0 right-0 flex justify-center gap-8 px-8'>
          {targets.map((target) => (
            <div
              ...
              style={{
                backgroundColor: target.filled
                  ? GAME_COLORS.slotFilled
                  : GAME_COLORS.slot,
                borderColor: target.filled ? '#16A34A' : '#94A3B8',
              }}
            >
              {target.filled
                ? renderShape(target.shape, target.color, 48)
                : renderShape(target.shape, target.color, 32)}
            </div>
          ))}
        </div>
