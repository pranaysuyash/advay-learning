...
function EarthTimeMachineContent() {
  ...
  const handleItemClick = useCallback((itemId: string) => {
    ...
    if (newState.status === 'success') {
      ...
      playError();
      void speak(`Oops! ${item.name} lived in the ${item.eraId.replace('-', ' ')} era.`);
    } else {
  ...
  if (subLoading) {
    return <GameShell gameId='earth-time-machine' gameName='Earth Time Machine'>Loading...</GameShell>;
  }

  if (!hasAccess) {
    return <AccessDenied gameName='Earth Time Machine' gameId='earth-time-machine' />;
  }
          ...
          {state.status === 'menu' && (
            ...
            >
              <div className='text-center'>
                <h1 className='text-4xl font-bold text-white mb-4'>Earth Time Machine</h1>
                <p className='text-xl text-blue-200 mb-8'>Travel through time and discover Earth's history!</p>
              </div>
          ...
          {state.status === 'playing' && currentChallenge && currentEra && (
            ...
            >
              ...
              <div className='w-full max-w-2xl bg-white/5 rounded-xl p-4'>
                <h3 className='text-white text-lg font-bold mb-4 text-center'>Time Travel Slider</h3>
                <div className='flex justify-between items-center gap-2'>
                  {ERAS.map((era) => (
                    ...
                    >
                      ...
                      <span className='text-xs text-white mt-1 font-medium'>{era.name}</span>
                    </motion.button>
                  ))}
                </div>
              </div>
              ...
              >
                ...
                <h2 className='text-3xl font-bold text-white mb-2'>{currentEra.name}</h2>
                  ...
                  {availableItems.map((item) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className='text-xs text-white mt-1'>{item.name}</div>
                      </motion.button>
                    );
                  })}
                </div>
              ...
              <div className='text-white/60 text-sm mt-4'>
                Found: {state.foundItems.length} / {currentChallenge.itemsToFind.length}
              </div>
            </motion.div>
          )}
            ...
            >
              ...
              {state.discoveredFacts.length > 0 && (
                <div className='bg-purple-500/20 rounded-xl p-4 max-w-lg'>
                  <h3 className='text-purple-200 font-bold mb-2'>Facts You Learned:</h3>
