...
const { BLOW_THRESHOLD, MIN_BLOW_DURATION, BLOW_COOLDOWN } = BUBBLE_GAME_CONFIG;
      ...
      setGameState((prev) => {
        ...
        if (newState.poppedCount > prev.poppedCount) {
          ...
          const newlyPoppedBubbles = newState.bubbles.filter(
            (b) =>
              b.isPopped &&
              !prev.bubbles.find((pb) => pb.id === b.id)?.isPopped,
          );
    ...
    (timestamp: number) => {
      ...
      if (deltaTime >= FRAME_TIME) {
        setGameState((prev) => {
          ...
          const { LEVEL_ADVANCE_POPS, LEVEL_ADVANCE_TIME_SECONDS, MAX_LEVEL } =
            BUBBLE_GAME_CONFIG;
          const popsForNextLevel = newState.level * LEVEL_ADVANCE_POPS;
          const timeForNextLevel = newState.level * LEVEL_ADVANCE_TIME_SECONDS;
          const elapsedTime =
            BUBBLE_GAME_CONFIG.GAME_DURATION_SECONDS - newState.timeLeft;

          if (
            newState.level < MAX_LEVEL &&
            (newState.poppedCount >= popsForNextLevel ||
              elapsedTime >= timeForNextLevel)
          ) {
            ...
            if (ttsEnabled && newState.level > lastAnnouncedLevelRef.current) {
              ...
              void speak(
                `Level ${newState.level}! Bubbles are getting faster!`,
              );
            }
          }
  ...
  useEffect(() => {
    ...
    const checkInactivity = setInterval(() => {
      ...
      const gameTimeElapsed =
        BUBBLE_GAME_CONFIG.GAME_DURATION_SECONDS - gameState.timeLeft;

      // Only encourage after game has started (3s grace period) and if inactive
      if (
        gameTimeElapsed > 3 &&
        timeSinceLastPop > 8000 &&
                  ...
                  <GameHUD
                    ...
                    levelInfo={
                      <div className='flex gap-4 items-center'>
                        ...
                        <div
                          className={`px-3 py-1 rounded-xl font-bold border-2 shadow-sm ${gameState.timeLeft < 10 ? 'bg-red-100 text-red-600 border-red-200 animate-pulse' : 'bg-slate-100 text-slate-600 border-slate-200'}`}
                        >
                ...
                <div className='absolute bottom-4 left-4 right-4 max-w-md mx-auto'>
                  <div className='bg-white rounded-2xl p-4 border-3 border-[#F2CC8F] shadow-[0_4px_0_#E5B86E]'>
                    <div className='flex items-center gap-3 mb-2'>
                      ...
                      <span className='font-bold text-advay-slate'>
                        Blow Power
                      </span>
                    </div>
                    <div className='h-6 bg-slate-100 rounded-full overflow-hidden border-2 border-[#F2CC8F]'>
                      <motion.div
                        ...
                        animate={{
                          width: `${Math.min(volume * 100 * 3, 100)}%`,
                        }}
                        transition={
                          reducedMotion ? { duration: 0 } : { duration: 0.1 }
                        }
                      />
                    </div>
                    {isBlowing && (
