...
const ColorSortGameGame = memo(function ColorSortGameGameComponent() {
  ...
  const startGame = () => {
    const { items: newItems, targets: newTargets } =
  ...
  return (
    ...
    >
      <div
        ref={gameAreaRef}
        className='flex flex-col items-center gap-4 p-4 h-full'
      >
        ...
        {gameState === 'start' && (
          <div className='text-center'>
            ...
            <button
              type='button'
              onClick={handleStart}
              className='px-8 py-4 bg-cyan-500 text-white rounded-2xl font-bold text-xl shadow-lg hover:scale-105 transition-transform'
            >
              Start!
            </button>
          </div>
        ...
        {gameState === 'playing' && (
          <div className='flex flex-col items-center gap-6'>
            ...
            <p className='text-lg font-bold'>
              Drag colors to matching buckets!
            </p>
            <div className='flex flex-wrap justify-center gap-3 relative'>
              {items.map((item, idx) => (
                <button
                  ...
                  className={`w-12 h-12 rounded-full shadow-md transition-transform ${
                    selectedItem === item
                      ? 'scale-125 ring-4 ring-black'
                      : 'hover:scale-110'
                  }`}
                ...
                >
                  ...
                  >
                    {buckets[target.name]?.map((item, idx) => (
                      <div
                        key={idx}
                        className='w-8 h-8 rounded-full shadow-sm animate-pulse'
                        style={{ backgroundColor: item.hex }}
                      />
                    ))}
                  </div>
        ...
        {gameState === 'complete' && (
          <div className='text-center'>
            ...
            <button
              type='button'
              onClick={handleStart}
              className='px-6 py-3 bg-cyan-500 text-white rounded-xl font-bold mr-4 hover:scale-105 transition-transform'
            >
              Play Again
            </button>
            <button
