...
function createSequenceRound(level: number): {
  ...
  const shuffledIds = targets
    .map((target) => target.id)
    .sort(() => randomFloat01() - 0.5);
  ...
  const handleFrame = useCallback(
    (frame: TrackedHandFrame, _meta: HandTrackingRuntimeMeta) => {
      ...
      if (nextStep >= orderRef.current.length) {
        ...
        setFeedback(
          `Great! Next shape ${nextStep + 1}/${orderRef.current.length}.`,
        );
        if (ttsEnabled) {
        ...
        {gameCompleted && (
          <div className='absolute inset-0 bg-slate-900/40 backdrop-blur-sm z-30 flex items-center justify-center'>
            <div className='bg-white border-3 border-[#F2CC8F] rounded-[3rem] p-12 text-center max-w-md w-[80%] shadow-[0_4px_0_#E5B86E]'>
              ...
              <h2 className='text-4xl font-black text-[#D946EF] tracking-tight mb-2'>
                Sequence Master!
              </h2>
              <p className='text-xl font-bold text-text-secondary mb-8'>
                Incredible job memorizing the order!
              </p>
