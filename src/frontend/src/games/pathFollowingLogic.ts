...
export function createPath(level: number): { path: PathPoint[]; config: LevelConfig } {
  const config = getLevelConfig(level);
  const path = generatePath(level);
  return { path, config };
}

export function isOnPath(x: number, y: number, path: PathPoint[], pathWidth: number): boolean {
