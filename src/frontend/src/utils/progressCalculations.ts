import { ProgressItem, UnifiedMetrics, HolisticScorecard, PlantGrowth } from '../types/progress';

/**
 * Calculate unified metrics across all activities
 */
export function calculateUnifiedMetrics(progress: ProgressItem[]): UnifiedMetrics {
  if (progress.length === 0) {
    return {
      ...
      challenge: { score: 0, level: 'comfortable', description: 'No challenge data' },
      consistency: { score: 0, level: 'inconsistent', description: 'No consistency data' },
    };
  }
  ...
  const practiceScore = Math.min(100, (avgActivitiesPerDay * 20) + (totalActivities * 2));
  const practiceLevel = practiceScore < 30 ? 'beginner' :
                       practiceScore < 70 ? 'intermediate' : 'advanced';

  // Mastery metric: Based on average accuracy and completion rates
  const avgAccuracy = progress.reduce((sum, p) => sum + p.score, 0) / progress.length;
  const highAccuracyCount = progress.filter(p => p.score >= 80).length;
  const masteryScore = (avgAccuracy * 0.7) + ((highAccuracyCount / progress.length) * 100 * 0.3);
  const masteryLevel = masteryScore < 40 ? 'emerging' :
                      masteryScore < 75 ? 'developing' : 'proficient';
  ...
  const scoreVariance = scores.length > 1 ?
    scores.reduce((sum, score) => sum + Math.pow(score - avgAccuracy, 2), 0) / scores.length : 0;
  const challengeScore = Math.min(100, avgAccuracy + (scoreVariance * 0.5));
  const challengeLevel = challengeScore < 50 ? 'comfortable' :
                        challengeScore < 80 ? 'stretching' : 'challenging';
  ...
  const consistencyScore = recentDays >= 7 ? 100 :
                          recentDays >= 5 ? 75 :
                          recentDays >= 3 ? 50 : 25;
  const consistencyLevel = consistencyScore < 40 ? 'inconsistent' :
                          consistencyScore < 80 ? 'regular' : 'consistent';

  return {
    practice: {
      score: Math.round(practiceScore),
      level: practiceLevel,
      description: getPracticeDescription(practiceLevel, totalActivities, avgActivitiesPerDay)
    },
    mastery: {
      score: Math.round(masteryScore),
      level: masteryLevel,
      description: getMasteryDescription(masteryLevel, avgAccuracy, highAccuracyCount)
    },
    challenge: {
 ...
 */
export function calculateHolisticScorecard(progress: ProgressItem[]): HolisticScorecard {
  const metrics = calculateUnifiedMetrics(progress);
  const weights = { practice: 0.25, mastery: 0.35, challenge: 0.2, consistency: 0.2 };
  ...
  if (metrics.practice.score >= 70) {
    strengths.push("Great practice habits - regular engagement shows commitment");
    insights.push("Consistent practice is building strong learning foundations");
  } else if (metrics.practice.score < 30) {
    areasForImprovement.push("Practice frequency could be increased for better progress");
    recommendations.push("Try setting a daily practice goal of 10-15 minutes");
  }

  if (metrics.mastery.score >= 75) {
    strengths.push("Excellent mastery - high accuracy shows deep understanding");
    insights.push("Strong grasp of concepts indicates effective learning strategies");
  } else if (metrics.mastery.score < 40) {
    areasForImprovement.push("Accuracy could be improved with focused practice");
    recommendations.push("Focus on one letter at a time until mastery before moving on");
  }

  if (metrics.challenge.score >= 80) {
    strengths.push("Good challenge balance - appropriately difficult activities");
    insights.push("Well-matched difficulty keeps learning engaging and effective");
  } else if (metrics.challenge.score < 50) {
    areasForImprovement.push("Activities might be too easy - could benefit from more challenge");
  ...
  if (metrics.consistency.score >= 80) {
    ...
    areasForImprovement.push("More consistent practice would accelerate progress");
    recommendations.push("Establish a regular practice schedule, even if shorter sessions");
  }
...
export function calculateHonestStats(progress: ProgressItem[]) {
  ...
  const tracingProgress = progress.filter(p => p.activity_type === 'letter_tracing');
  const avgTracingAccuracy = tracingProgress.length > 0
    ? tracingProgress.reduce((sum, p) => sum + p.score, 0) / tracingProgress.length
    : 0;
  ...
  const recentActivity = progress
    ...
    .slice(0, 10)
    .map(item => ({
      action: item.activity_type === 'letter_tracing'
...
function getPracticeDescription(level: string, totalActivities: number, avgPerDay: number): string {
  switch (level) {
    case 'beginner':
...
function getMasteryDescription(level: string, avgAccuracy: number, highAccuracyCount: number): string {
  switch (level) {
    case 'emerging':
