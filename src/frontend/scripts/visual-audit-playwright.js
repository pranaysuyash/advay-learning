...
const PAGES = [
  ...
  { name: '04-dashboard', path: '/dashboard', waitFor: '[data-testid="dashboard"], .dashboard, h1', auth: true },
  { name: '05-games', path: '/games', waitFor: '.grid, [data-testid="games"]', auth: true },
  { name: '06-alphabet-game-menu', path: '/games/alphabet-tracing', waitFor: 'button, .game', auth: true },
  { name: '07-progress', path: '/progress', waitFor: '[data-testid="progress"], .progress', auth: true },
  { name: '08-settings', path: '/settings', waitFor: '[data-testid="settings"], form, .settings', auth: true },
];
...
async function runAudit() {
  ...
  for (const viewport of VIEWPORTS) {
    ...
    for (const pageConfig of PAGES) {
      ...
      // For non-auth pages after login, we might want to navigate fresh
      if (!pageConfig.auth && isLoggedIn) {
        // Continue with logged-in state (shows user menu, etc.)
      }
