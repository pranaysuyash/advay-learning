...
2. **Head Tilt Maze**: Tilt head L/R to roll a ball through a maze.
3. **Blinky Bird**: Blink to make a bird fly over pipes.
4. **Statue Freeze**: Move when music plays, freeze when it stops (Pose detection).
5. **Emotion Mirror**: "Show me a Happy Face" (Face landmark detection).
