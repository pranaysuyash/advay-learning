 ...
 */
async function getFeaturedGamesEnhanced(profile?: Profile): Promise<GameManifest[]> {
  ...
  const capableGames = allGames.filter(game => {
    ...
    if (game.cvRequirements?.camera && !deviceCapabilities.hasCamera) return false;
    if (game.cvRequirements?.microphone && !deviceCapabilities.hasMicrophone) return false;
  ...
  const unplayed = capableGames.filter(g => 
    !playedIds.has(g.id) && !slots.includes(g)
  );
  ...
  if (unplayed.length > 0) {
    ...
    const vibeMatches = (qualityUnplayed.length ? qualityUnplayed : unplayed)
      .filter(g => g.vibe === preferredVibe);
    
    // Prefer vibe matches, fallback to any quality unplayed
    const pool = vibeMatches.length > 0 ? vibeMatches : 
                 (qualityUnplayed.length > 0 ? qualityUnplayed : unplayed);
...
function ensureWorldDiversity(games: GameManifest[]): GameManifest[] {
  ...
  for (const game of games) {
    ...
    if (count < 2) { // Max 2 games from same world
      diverse.push(game);
...
interface GlobalGameStatsResponse {
  stats: {
    ...
    ageCohortRank: {                 // Rankings per age group (1 = most popular)
      '3-4y'?: number;
      '5-6y'?: number;
      '7-8y'?: number;
    };
...
export function AlphabetTracing() {
  const { recordGameSessionProgress } = useGameSessionProgress('alphabet-tracing');
...
export const useProgressStore = create<ProgressState>()(
  persist(
    (set, get) => ({
      ...
      recordGamePlay: (profileId, gameId, durationMinutes, completed, score = 0) => {
        set((state) => {
          ...
          if (existing) {
            ...
            const newCompletionRate = 
              (existing.completionRate * existing.playCount + (completed ? 1 : 0)) / newPlayCount;
...
import { getFeaturedGamesEnhanced, getAgeGroup } from '../utils/recommendations';
...
export function Dashboard() {
  ...
  const getBadgeForSlot = (game: GameManifest, slotIndex: number): GameCardBadge => {
    ...
    switch (slotIndex) {
      ...
      case 2: // TRENDING (global popularity)
        if (stats) {
          const ageGroup = currentProfile?.age ? getAgeGroup(currentProfile.age) : null;
      ...
      case 3: // DISCOVER (unplayed + quality)
        ...
        return {
          icon: '🎲',
          label: 'EXPLORE',
          subtitle: gameHistoryEntry ? undefined : 'You haven\'t tried this yet!',
          color: 'purple'
        };
...
import { getFeaturedGamesEnhanced, getAgeGroup, ensureWorldDiversity } from '../recommendations';
import { useProgressStore } from '../../store/progressStore';
...
describe('getFeaturedGamesEnhanced', () => {
  ...
  it('filters games by device capabilities', async () => {
    // Mock no camera
    vi.spyOn(navigator.mediaDevices, 'getUserMedia').mockRejectedValue(new Error('No camera'));
...
describe('ensureWorldDiversity', () => {
  it('limits max 2 games from same world', () => {
    ...
    const letterCount = diverse.filter(g => g.worldId === 'letter-land').length;
