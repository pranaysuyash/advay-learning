...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│                         LANGUAGE SYSTEM CHAOS                               │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  1. Profile.preferred_language     2. settings.gameLanguage                │
│     (Stored in DB)                   (Stored in localStorage)               │
│                                                                             │
│         'en'                              'english'                         │
│          │                                     │                            │
│          ▼                                     ▼                            │
│  Games.tsx DISPLAYS                 AlphabetGame.tsx USES                   │
│  "Play in English"                   (but different value!)                 │
│                                                                             │
│  3. settings.language                                                       │
│     (UI language - not used for game)                                       │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
|-----------|--------|-------|
| `Profile.preferred_language` field | ✅ | Stores 'en', 'hi', 'kn', 'te', 'ta' |
| `ProfileUpdate` schema | ✅ | Has preferred_language field |
| `ProfileService.update()` | ✅ | Can update profile |
| **API Endpoint** | ❌ **MISSING** | No PATCH /me/profiles/{id} endpoint |

### Frontend (⚠️ Broken)

| Component | Uses | Status | Problem |
|-----------|------|--------|---------|
| **Dashboard** | `preferred_language` | ✅ Works | But NO EDIT functionality |
| **Games.tsx** | `preferred_language` | ✅ Display | Shows language badge correctly |
| **AlphabetGame.tsx** | `settings.gameLanguage` | ❌ **WRONG** | Ignores profile, uses settings |
| **Settings Store** | `gameLanguage` | ⚠️ Confusing | Separate from profile |
...
User Creates Profile
        ...
        ▼
┌───────────────┐     ┌─────────────────┐     ┌──────────────────┐
│   Dashboard   │────→│  Profile Store  │────→│   Backend DB     │
│               │     │preferred_language│     │ (stores 'en')    │
└───────────────┘     └─────────────────┘     └──────────────────┘
                              │
                              ▼
                       Games.tsx DISPLAYS
...
## 🎯 What Fixing TCK-20260129-301 Unlocks
