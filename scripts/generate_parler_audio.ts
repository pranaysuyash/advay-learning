/**
 * generate_parler_audio.ts
 *
 * Pre-generates high-quality TTS audio clips for ALL statically-known spoken
 * text across every game. Uses HuggingFace Parler TTS inference API,
 * avoiding any local ONNX/Node model execution which was blocked by macOS EPERM.
 *
 ...
 * Usage:
 *   export HF_TOKEN="hf_..."
 *   npx tsx scripts/generate_parler_audio.ts
 *
 * Output:
...
const RATE_LIMIT_MS = 1500; // HF free tier needs breathing room
const FORCE = process.argv.includes('--force');

const HF_TOKEN = process.env.HF_TOKEN;
if (!HF_TOKEN) {
    console.error('❌ Set HF_TOKEN environment variable first. Get one at huggingface.co/settings/tokens');
    process.exit(1);
}
...
const VOICE_DESCRIPTION =
    ...
    "Her voice is warm, clear, and child-friendly. " +
...
async function generateAudioParler(text: string): Promise<ArrayBuffer> {
    const models = [
        'parler-tts/parler-tts-mini-v1',
        'facebook/mms-tts-eng', // fast fallback
    ];
    ...
    for (const model of models) {
        try {
            ...
            if (!response.ok) {
                ...
                // 503 means model is loading, wait and throw so outer loop can retry
                if (response.status === 503) {
                    throw new Error(`Model loading: ${errText.slice(0, 50)}`);
                }
                console.warn(`⚠️  ${model} returned ${response.status}: ${errText.slice(0, 50)}`);
                continue; // try next model
            }

            const audioBuffer = await response.arrayBuffer();
            if (audioBuffer.byteLength < 1000) {
                console.warn(`⚠️  Skipping tiny audio: ${audioBuffer.byteLength} bytes`);
                continue;
            }

            return audioBuffer;
        } catch (err) {
            if ((err as Error).message.includes('Model loading')) {
                throw err; // bubble up 503 so we retry the whole phrase instead of failing over to MMS instantly
            }
...
async function main() {
    console.log('🎵  generate_parler_audio — Pre-generate Full Game Audio via HuggingFace');
    ...
    for (const [normalized, original] of corpus.entries()) {
        ...
        // Infinite retry loop for 503 Model Loading
        let success = false;
        let retries = 0;
        while (!success && retries < 5) {
            try {
                const buf = await generateAudioParler(original);
                ...
                if ((err as Error).message.includes('Model loading')) {
                    ...
                    await new Promise((r) => setTimeout(r, 10000)); // wait 10s for HF model to spin up
                    retries++;
                } else {
