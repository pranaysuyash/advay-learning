...
def relative_luminance(rgb: tuple[int, int, int]) -> float:
    ...
    def channel_luminance(c: int) -> float:
        ...
        if srgb <= 0.03928:
            return srgb / 12.92
        return ((srgb + 0.055) / 1.055) ** 2.4  # type: ignore[no-any-return]
...
def main():
    ...
    results.sort(key=lambda x: x["ratio"])  # type: ignore[arg-type, return-value]

    # Print failing combinations first
    print("⚠️  FAILING COMBINATIONS (Need Attention)")
    print("-" * 80)
    failing = [r for r in results if "FAIL" in r["level"]]  # type: ignore[operator]
    if failing:
        for r in failing:
            print(f"  {r['description']}")
            print(
                f"    {r['text']} ({colors[r['text']]}) on {r['bg']} ({colors[r['bg']]})"  # type: ignore[index]
            )
    ...
    print("-" * 80)
    aa_large = [r for r in results if "AA Large" in r["level"]]  # type: ignore[operator]
    if aa_large:
        for r in aa_large:
            print(f"  {r['description']}")
    ...
    print("-" * 80)
    passing = [r for r in results if "AA" in r["level"] and "Large" not in r["level"]]  # type: ignore[operator]
    for r in passing:
        status = "🌟" if "AAA" in r["level"] else "✓"  # type: ignore[operator]
    ...
    print(
        f"  ❌ Failing (< 3:1):        {fail_count} ({fail_count / total * 100:.1f}%)"
    )
    print(
        f"  ⚡ AA Large only (3-4.5):  {aa_large_count} ({aa_large_count / total * 100:.1f}%)"
    )
    print(
        f"  ✅ AA Pass (4.5-7):       {len([r for r in results if 4.5 <= r['ratio'] < 7])} ({len([r for r in results if 4.5 <= r['ratio'] < 7]) / total * 100:.1f}%)"  # type: ignore[operator]
    )
    print(
        f"  🌟 AAA Enhanced (7+):      {len([r for r in results if r['ratio'] >= 7])} ({len([r for r in results if r['ratio'] >= 7]) / total * 100:.1f}%)"  # type: ignore[operator]
    )
    print()
