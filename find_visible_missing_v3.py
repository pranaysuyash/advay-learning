...
for file in files:
    ...
    # Open with UTF-8 encoding to handle emoji in registry files
    ...
    for game_id in blocks:
        ...
        if brace_end != -1:
            ...
            # Check for listed: true as boolean (not just string match)
            # Handle both "listed: true" and "listed:true" and "const listed = true"
            is_listed = (
                re.search(r'listed:\s*true', block) is not None or
                re.search(r'listed:\s*LISTED', block) is not None or
                (re.search(r'const\s+LISTED\s*=\s*true', content) is not None and 'listed: LISTED' in block)
            )
            if is_listed and "previewImage:" not in block:
